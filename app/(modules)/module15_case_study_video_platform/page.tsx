...
export default function Module15Page() {
  ...
  const mod = getModuleBySlug('module15_case_study_video_platform');
  ...
  return (
    <div className="min-h-screen bg-background-light dark:bg-background-dark">
      <section className="relative py-16 overflow-hidden">
        <div className="absolute inset-0 bg-gradient-to-br from-red-600/10 to-red-800/5" />
        <div className="container-custom relative">
          <div className="flex items-center gap-4 mb-4">
            <div className={`w-14 h-14 rounded-2xl bg-gradient-to-br ${mod.color} flex items-center justify-center text-white font-bold text-xl shadow-lg`}>15</div>
      ...
      <div className="container-custom py-12">
        <div className="space-y-12">
          {mod.topics.map((topic, i) => (
            <div key={i} className="glass-card p-6 md:p-8">
              ...
              <div className="p-5 bg-red-950/30 border border-red-500/20 rounded-xl mb-6" dir="rtl">
                <h4 className="text-lg font-bold text-red-400 font-arabic mb-3">📖 {topic.title_ar}</h4>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  {topic.keyPoints.map((kp, j) => (
                    <div key={j} className="p-4 rounded-xl bg-gray-800/30 border border-gray-700/30">
                      <h5 className="font-bold text-red-400 mb-1">{kp.title}</h5>
        ...
        <div className="flex justify-between mt-12">
          <Link href="/module14_case_study_social_media" className="btn-outline px-6 py-3 rounded-xl">← Social Media Feed</Link>
          <Link href="/module16_interview_framework" className="btn-primary px-6 py-3 rounded-xl">Next: Interview Framework →</Link>
        </div>
      </div>
    </div>
  );
